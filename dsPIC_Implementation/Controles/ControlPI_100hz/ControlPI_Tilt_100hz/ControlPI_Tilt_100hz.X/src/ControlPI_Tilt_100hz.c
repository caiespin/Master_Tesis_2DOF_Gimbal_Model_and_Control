...
#include "ControlPI_Tilt_100hz.h"
#include "ControlPI_Tilt_100hz_private.h"

/* Block signals (auto storage) */
B_ControlPI_Tilt_100hz_T ControlPI_Tilt_100hz_B;

/* Block states (auto storage) */
DW_ControlPI_Tilt_100hz_T ControlPI_Tilt_100hz_DW;

/* Real-time model */
RT_MODEL_ControlPI_Tilt_100hz_T ControlPI_Tilt_100hz_M_;
RT_MODEL_ControlPI_Tilt_100hz_T *const ControlPI_Tilt_100hz_M =
  &ControlPI_Tilt_100hz_M_;

/* Model step function for TID0 */
void ControlPI_Tilt_100hz_step0(void)  /* Sample time: [0.001s, 0.0s] */
{
  /* (no output/update code required) */
}

/* Model step function for TID1 */
void ControlPI_Tilt_100hz_step1(void)  /* Sample time: [0.01s, 0.0s] */
{
  ...
  /* MCHP_UART_Rx Block for UARTRef 1: <Root>/UART Rx/Outputs */
  {
    {
      int_T i1;
      uint8_T *y0 = &ControlPI_Tilt_100hz_B.U1Rx[0] ;
      for (i1 = 0; i1 < 42 ; i1++) {
  ...
  ControlPI_Tilt_100hz_B.CFunctionCall = Receive_wTy(
    &ControlPI_Tilt_100hz_B.U1Rx[0]
    );
   ...
   *  Product: '<S2>/Product'
   */
  rtb_Sum = 500.0 * ControlPI_Tilt_100hz_B.CFunctionCall +
    ControlPI_Tilt_100hz_DW.DiscreteTimeIntegrator_DSTATE;
  ...
  if (rtb_Sum_0 <= 3997U) {
    ControlPI_Tilt_100hz_B.Saturation = rtb_Sum_0;
  } else {
    ControlPI_Tilt_100hz_B.Saturation = 3997U;
  }
  ...
  ControlPI_Tilt_100hz_DW.DiscreteTimeIntegrator_DSTATE += 9.0 *
    ControlPI_Tilt_100hz_B.CFunctionCall;

  /* Update for S-Function (MCHP_PWM): '<S1>/PWM' */
  P1DC2 = ControlPI_Tilt_100hz_B.Saturation;/* Period for Channel 2 */
  ...
  /* MCHP_UART_Tx Block for UARTRef 2: <Root>/UART Tx/Outputs */
  {
    ...
    {
      int_T i1;
      uint8_T *u0 = &ControlPI_Tilt_100hz_B.U1Rx[0] ;
      for (i1 = 0; i1 < 42 ; i1++) {
        if (Tmp != 0) {
...
void ControlPI_Tilt_100hz_step(int_T tid)
{
  switch (tid) {
   case 0 :
    ControlPI_Tilt_100hz_step0();
    break;

   case 1 :
    ControlPI_Tilt_100hz_step1();
    break;
...
void ControlPI_Tilt_100hz_initialize(void)
{
  /* Registration code */
  ...
  /* block I/O */
  (void) memset(((void *) &ControlPI_Tilt_100hz_B), 0,
                sizeof(B_ControlPI_Tilt_100hz_T));

  /* states (dwork) */
  (void) memset((void *)&ControlPI_Tilt_100hz_DW, 0,
                sizeof(DW_ControlPI_Tilt_100hz_T));
